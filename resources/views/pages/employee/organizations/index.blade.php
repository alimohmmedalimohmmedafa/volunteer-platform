@extends('layouts.employee')

@section('title', 'المنظمات')

@section('content')

<div class="mb-4">
    <h2 class="fw-bold mb-1">المنظمات</h2>
    <p class="text-muted mb-0">مراجعة وإدارة منظمات المنصة</p>
</div>

{{-- Status filter tabs --}}
<ul class="nav nav-pills mb-4">

    <li class="nav-item">
        <a href="{{ route('employee.organizations.index') }}"
           class="nav-link {{ is_null($status) ? 'active' : '' }}">
            الكل
        </a>
    </li>

    <li class="nav-item">
        <a href="{{ route('employee.organizations.index', ['status' => 'pending']) }}"
           class="nav-link {{ $status === 'pending' ? 'active' : '' }}">
            قيد المراجعة
        </a>
    </li>

    <li class="nav-item">
        <a href="{{ route('employee.organizations.index', ['status' => 'approved']) }}"
           class="nav-link {{ $status === 'approved' ? 'active' : '' }}">
            معتمدة
        </a>
    </li>

    <li class="nav-item">
        <a href="{{ route('employee.organizations.index', ['status' => 'rejected']) }}"
           class="nav-link {{ $status === 'rejected' ? 'active' : '' }}">
            مرفوضة
        </a>
    </li>

</ul>

<div class="card border-0 shadow-sm">

    <div class="table-responsive">

        <table class="table table-hover align-middle mb-0">

            <thead class="table-light">

                <tr>
                    <th>المنظمة</th>
                    <th>البريد</th>
                    <th>المدينة</th>
                    <th>الحالة</th>
                    <th>التاريخ</th>
                    <th></th>
                </tr>

            </thead>

            <tbody>

                @forelse($organizations as $organization)

                    <tr>

                        <td>
                            <strong>{{ $organization->name }}</strong>
                        </td>

                        <td>{{ $organization->email }}</td>

                        <td>{{ $organization->city }}</td>

                        <td>

                            @if($organization->status === 'pending')

                                <span class="badge bg-warning text-dark">قيد المراجعة</span>

                            @elseif($organization->status === 'approved')

                                <span class="badge bg-success">معتمدة</span>

                            @else

                                <span class="badge bg-danger">مرفوضة</span>

                            @endif

                        </td>

                        <td>{{ $organization->created_at->format('Y-m-d') }}</td>

                        <td>

                            <a href="{{ route('employee.organizations.show', $organization) }}"
                               class="btn btn-sm btn-outline-primary">
                                مراجعة
                            </a>

                        </td>

                    </tr>

                @empty

                    <tr>

                        <td colspan="6" class="text-center py-5">

                            <i class="bi bi-building display-5 text-muted"></i>

                            <p class="text-muted mt-3 mb-0">لا توجد منظمات.</p>

                        </td>

                    </tr>

                @endforelse

            </tbody>

        </table>

    </div>

</div>

<div class="mt-4">
    {{ $organizations->links() }}
</div>

@endsection